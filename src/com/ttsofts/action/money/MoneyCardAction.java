...
public class MoneyCardAction extends BaseAbstractAction {
	...
	public String findByAll() {
		...
		List<MoneyCard> list = moneyCardMapper.findByAll();
	...
	public String add() {
		...
		try {
			...
			MoneyCard entity = JSON.parseObject(param,MoneyCard.class);
			int b = moneyCardMapper.add(entity);
			if (b>0) {
				result.put("result", true);
				result.put("msg", "添加成功");
			} else {
	...
	public String update() {
		...
		try {
			param = URLDecoder.decode(param,"UTF-8");
			MoneyCard entity = JSON.parseObject(param,MoneyCard.class);
			int b = moneyCardMapper.update(entity);
			if (b>0) {
				result.put("result", true);
				result.put("msg", "添加成功");
			} else {
	...
	public String findByMap() {
		// TODO Auto-generated method stub
		result = new HashMap<String, Object>();
		Object obj = JSON.parseObject(param, MoneyCard.class);
		Map<String, Object> map;
		try {
			map = BeanObjectToMap.convertBean(obj);
			List<MoneyCard> list =  moneyCardMapper.findByMap(map);
			if (list == null || list.size() == 0) {
				result.put("result", false);
				result.put("msg", "没有查询到结果");
			} else {
	...
	public String delete() {
		...
		try {
			...
			int b = moneyCardMapper.deleteById(entity.getId());
			if (b>0) {
				result.put("result", true);
				result.put("msg", "删除成功");
	...
	public String findByPage() {
		...
		try {
			...
			List<MoneyCard> list = moneyCardMapper.findByPage(pageMap);
			Map<String,Object> infoMap = new HashMap<String,Object>();
			int totalCount = moneyCardMapper.getCount(infoMap);
	...
	public String getCount() {
		...
		try {
			...
			int b = moneyCardMapper.getCount(map);
			result.put("result", true);
			result.put("count", b);
		} catch (Exception e) {
			result.put("result", false);
