...
public class ProxyHandler extends HttpServlet {
	...
	private void noSecurityRequest(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		try {
			...
			if (null != req.getHeader("Referer"))
				conn.setRequestProperty("Referer", req.getHeader("Referer"));
			 ...
			 * .getInputStream(), CHARSET_UTF8)); BufferedWriter wd = new
			 * BufferedWriter(new OutputStreamWriter(resp .getOutputStream(),
			 * CHARSET_UTF8));
			 * 
			 * String tempLine = rd.readLine(); while (tempLine != null) {
			 * wd.write(tempLine); logger.info(tempLine); tempLine =
			 * rd.readLine(); } wd.flush();
			 */
