...
require(options, function(base, etpl) {
	...
	app.showModel=function(){
		...
		$("#okBtn").on("click",function(evt){
			...
			obj.money = $("#money").val();
			obj.loseDate = $("#loseDate").val();
			obj.repaymentDate = $("#repaymentDate").val();
			obj.validateId = $("#validateId").val();
			var id = $.trim($("#id").val());
			if(id===""){
				that.save(obj);
			}else{
		...
		base.sendPost(this.baseurl + "findByPage", JSON.stringify(obj),function(json){
			if(json.result){
				...
				for(var i = 0;i < count;i++){
					totalmoney+=list[i].money;
				}
				$("#totalcount").html("共"+count+"条数据");
				$("#totalmoney").html("共"+totalmoney+"条数据");
				if(count>0){
	...
	app.findByAll = function(){
		...
		base.sendGet(this.baseurl + "findByAll", function(json) {
			if(json.result){
				...
				//$("#totalcount").html("共"+count+"条数据");
				$("#totalmoney").html("总额："+totalmoney+"元");
				if(count>0){
					html =that.listRender(json);
				}
				$("#listBody").html(html);
